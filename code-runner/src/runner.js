...
function formatCmd(template, filePath) {
    const base = filePath.replace(/\.[^/.]+$/, '');
    return template.replace(/\{file\}/g, filePath).replace(/\{base\}/g, base);
}
export async function runCodeLocal({ code, language, testCases, filenamePrefix, input = '' }) {
    ...
    if (lang.compile) {
        ...
        if (compileRes.exitCode !== 0)
            return testCases ? [compileRes] : compileRes;
    }
    const runCmd = formatCmd(lang.run, filePath);
    if (testCases) {
        let results = [];
        if (testCases.parallel) {
            const tasks = testCases.cases.map((test) => async () => {
...
export async function runCodeDocker({ code, language, testCases, filenamePrefix, input = '' }) {
    const lang = LANGS[language];
    ...
    function formatDockerTimeMem(res) {
        let timeMs = 0;
        let memoryKb = 0;
        ...
        if (memMatch)
            memoryKb = parseInt(memMatch[1]);
        return { ...res, timeMs, memoryKb };
    }
}
function execAsync(cmd, input = '') {
